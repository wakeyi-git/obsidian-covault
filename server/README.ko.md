...
매핑; TLS는 Cloudflare 엣지에서 종단되고 WebSocket도 지원. CGNAT 환경에 이상적. 엣지 로깅/분석에서 쿼리
문자열을 제외하는 것은 선택적 심층 방어입니다 — CoVault는 실시간 토큰을 URL이 아닌 인증 메시지로 보냅니다.
...
운영자 vault는 **구성원 DB마다 live `_changes` longpoll 1개**(+ 공동 공간마다 1개)를 상시 유지합니다.
구성원이 30~40명이면 HTTP/1.1의 호스트당 동시 연결 제한(보통 6)을 넘어 요청이 줄을 서고 동기화가 늦어집니다. 완화책:

- CouchDB 앞에 **HTTP/2**를 말하는 리버스 프록시를 두세요(Caddy·최신 nginx는 HTTPS에서 자동) —
  멀티플렉싱이 호스트당 연결 상한을 없앱니다.
- 또는 플러그인 운영자 설정의 **통합 변경 감지**(실험적, v0.112+)를 켜세요 — `_db_updates` 연결 1개로 전체 DB를
  감시하고 변경된 DB만 동기화합니다. 서버 관리자 자격증명이 필요하며 미지원이면 자동으로 기존 방식으로 동작합니다.
- 운영자 기기는 데스크톱 앱에서 큰 워크스페이스를 돌리고, 모바일은 구성원 vault 위주로 쓰세요
  (구성원 vault는 연결을 1~2개만 씁니다).

시작 시에는 구성원 폴더마다 전체 파일을 한 번 훑습니다(해시 검사) — 설치 직후 첫 동기화는 대용량 vault에서
몇 분 걸릴 수 있습니다.
...
- [ ] (심층 방어) 리버스 프록시/CDN이 쿼리 문자열을 로그에 남기지 않음 — CoVault는 토큰을 URL에 싣지 않지만 다른 앱이 실을 수 있습니다.
